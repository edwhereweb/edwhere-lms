...
export const QuizPlayer = ({ courseId, chapterId, quizConfig }: QuizPlayerProps) => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const res = await axios.get(`/api/courses/${courseId}/chapters/${chapterId}/quiz/start`);
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
         ...
         if (error.response?.status === 403) {
            ...
            toast.error(error.response?.data?.error || 'Access forbidden');
         } else {
  ...
  const startTest = async () => {
     setStarting(true);
     try {
        const res = await axios.post(`/api/courses/${courseId}/chapters/${chapterId}/quiz/start`);
        const attempt = res.data;
        setSession(attempt);
        setHasStarted(true);
        setTabSwitches(Math.max(attempt.tabSwitches || 0, 0));
  ...
  const forceSubmit = async (finalTabSwitches: number) => {
      ...
      try {
          const res = await axios.post(`/api/courses/${courseId}/chapters/${chapterId}/quiz/submit`, {
  ...
  const handleOptionToggle = (questionId: string, optionIndex: number, isMultipleChoice: boolean) => {
  ...
  const debouncedSave = useCallback((questionId: string, selections: number[]) => {
      ...
      debounceTimers.current[questionId] = setTimeout(async () => {
         try {
             await axios.post(`/api/courses/${courseId}/chapters/${chapterId}/quiz/save`, {
                 attemptId: session.id,
                 questionId,
  ...
  if (!hasStarted) {
      ...
      return (
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-8 rounded-md text-center max-w-2xl mx-auto mt-8 shadow-sm">
              ...
              <div className="text-left space-y-4 text-slate-600 dark:text-slate-300 mb-8 border-y py-6">
                 <p>• <strong className="text-slate-900 dark:text-slate-100">Grading:</strong> {quizConfig.isGraded ? 'This is a graded test. Your score will be recorded.' : 'This is a practice quiz.'}</p>
                 <p>• <strong className="text-slate-900 dark:text-slate-100">Time Limit:</strong> {quizConfig.timeLimit ? `${quizConfig.timeLimit} minutes` : 'No time limit'}. The timer will begin immediately when you click start.</p>
                 {quizConfig.maxAttempts ? <p>• <strong className="text-slate-900 dark:text-slate-100">Attempts:</strong> You have a maximum of {quizConfig.maxAttempts} attempts.</p> : null}
                 {quizConfig.maxTabSwitches ? <p>• <strong className="text-slate-900 dark:text-slate-100">Anti-Cheat:</strong> Tab switching is strictly monitored. Moving away from the test window {quizConfig.maxTabSwitches} times will automatically submit your test.</p> : null}
              </div>
              <Button onClick={startTest} disabled={starting} size="lg" className="w-full sm:w-auto h-12 px-8 text-lg font-semibold bg-indigo-600 hover:bg-indigo-700 text-white">
                  {starting ? <Loader2 className="h-5 w-5 animate-spin mr-2" /> : null}
                  Start Test Now
              </Button>
          </div>
  ...
  if (!session) {
      return (
         <div className="bg-rose-50 border border-rose-200 text-rose-700 p-6 rounded-md flex items-center justify-center">
            ...
            <p>You cannot access this evaluation (Maximum attempts reached or configuration missing).</p>
         </div>
      );
  }
  ...
  return (
      <div className="pb-20 select-none">
             ...
             <div className="flex items-center gap-x-4">
                 {timeLeft !== null && (
                    <div className={cn(
                        ...
                        timeLeft < 60 ? "bg-rose-100 text-rose-700 animate-pulse" : "bg-slate-100 dark:bg-slate-800"
                    )}>
          ...
          <div className="space-y-8">
              {questions.length > 0 && (
                  <div key={questions[currentQuestionIndex].id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-md">
                      <div className="font-medium text-lg mb-4 flex">
                         <span className="text-indigo-500 mr-2">Question {currentQuestionIndex + 1} of {questions.length}:</span> 
                      </div>
                      ...
                      <div className="space-y-3 pl-6">
                         ...
                         {questions[currentQuestionIndex].options.map((opt: any) => {
                             const isChecked = (responses[questions[currentQuestionIndex].id] || []).includes(opt.originalIndex);
                             return (
                                 <div 
                                    key={opt.originalIndex}
                                    onClick={() => handleOptionToggle(questions[currentQuestionIndex].id, opt.originalIndex, questions[currentQuestionIndex].isMultipleChoice)}
                                    className={cn(
                                    ...
                                    <div className="mr-4 flex-shrink-0">
                                        {questions[currentQuestionIndex].isMultipleChoice ? (
                                           ...
                                           <div className={cn(
                                               ...
                                               isChecked ? "border-indigo-500" : "border-slate-300 dark:border-slate-700"
                                           )}>
                                               {isChecked && <div className="h-2.5 w-2.5 rounded-full bg-indigo-500" />}
                                           </div>
                                        )}
                                    </div>
                                    <span className={cn("text-base", isChecked && "font-medium text-indigo-900 dark:text-indigo-200")}>
                                        {opt.text}
                                    </span>
                                 </div>
                             );
