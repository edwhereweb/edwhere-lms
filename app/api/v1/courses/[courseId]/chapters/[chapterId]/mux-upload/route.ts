...
/**
 * POST /api/v1/courses/[courseId]/chapters/[chapterId]/mux-upload
 *
...
export async function POST(
  ...
  try {
    ...
    if (!chapter) return apiErr('NOT_FOUND', 'Chapter not found', 404);
    ...
    return apiOk({ uploadUrl: upload.url, uploadId: upload.id });
  } catch (error) {
    return handleRouteError('CHAPTER_MUX_UPLOAD_CREATE', error);
  }
}
