...
import { handleRouteError, apiErr, apiOk } from '@/lib/api-response';
...
export async function GET(
  ...
  try {
    ...
    if (!quiz) return apiErr('NOT_FOUND', 'Quiz not found', 404);
    ...
    if (existingAttempt) {
      ...
      return apiOk({ ...existingAttempt, questions: processedQuestions });
    }
    ...
    return apiOk({ activeAttempt: null, previousAttempts });
  } catch (error) {
    return handleRouteError('CHAPTER_QUIZ_START_GET', error);
  }
}
  ...
  try {
    ...
    if (quiz.maxAttempts) {
      ...
      if (count >= quiz.maxAttempts) {
        return apiErr('FORBIDDEN', 'Maximum attempts reached.', 403);
      }
    }
    ...
    return apiOk({ ...newAttempt, questions: processedQuestions });
  } catch (error) {
    return handleRouteError('CHAPTER_QUIZ_START', error);
  }
}
