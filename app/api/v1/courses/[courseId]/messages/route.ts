...
// GET /api/v1/courses/[courseId]/messages?threadStudentId=xxx
export async function GET(req: Request, { params }: Params) {
  try {
    ...
    if (!resolvedThreadId) {
      return apiErr('VALIDATION', 'threadStudentId required', 400);
    }
    ...
    return handleRouteError('MESSAGES_GET', error);
  }
}
...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    const validation = validateRequest(messageBodySchema, body);
    ...
    if (!threadStudentId) {
      return apiErr('VALIDATION', 'threadStudentId required for instructors', 400);
    }
    ...
    return handleRouteError('MESSAGES_POST', error);
  }
}
