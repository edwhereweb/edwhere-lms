...
const BloggerEditLandingPage = async ({ params }: { params: { pageId: string } }) => {
  const { userId } = await auth();
  if (!userId) return redirect('/');

  const profile = await getSafeProfile();
  if (!profile || !['BLOGGER', 'ADMIN'].includes(profile.role)) return redirect('/');

  const landingPage = (await db.landingPage.findUnique({
    where: { id: params.pageId, createdBy: userId }
  })) as unknown as LandingPage;

  if (!landingPage) return redirect('/blogger/landing-pages');
...
export default BloggerEditLandingPage;
