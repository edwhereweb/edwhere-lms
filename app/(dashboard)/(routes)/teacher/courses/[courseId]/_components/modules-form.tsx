...
interface ModulesFormProps {
  initialData: Course & {
    modules: (Module & { chapters: Chapter[] })[];
  };
  courseId: string;
}
...
export const ModulesForm = ({ initialData, courseId }: ModulesFormProps) => {
  ...
  return (
    <div className="relative mt-6 border bg-slate-100 rounded-md p-4 dark:bg-gray-800">
      {isUpdating && (
        <div className="absolute h-full w-full bg-slate-500/20 top-0 right-0 rounded-md flex items-center justify-center z-10">
      ...
      {!isCreating && (
        <div className={cn('text-sm mt-2', !initialData.modules.length && 'text-slate-500 italic')}>
      ...
      {!isCreating && (
        <p className="text-xs text-muted-foreground mt-4">Drag and drop to reorder the modules</p>
      )}
    </div>
  );
};
