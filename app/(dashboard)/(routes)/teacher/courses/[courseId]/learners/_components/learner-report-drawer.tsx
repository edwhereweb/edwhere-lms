...
export const LearnerReportDrawer = ({
  isOpen,
  onClose,
  courseId,
  studentId,
  learnerName
}: LearnerReportDrawerProps) => {
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/courses/${courseId}/analytics/learner/${studentId}`);
        setData(res.data);
      } catch {
        setData(null);
      } finally {
    ...
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto z-[9999]" side="right">
        <SheetHeader className="mb-8">
          ...
          <SheetDescription>
            Performance and progress metrics for{' '}
            <span className="font-semibold text-slate-800 dark:text-slate-200">{learnerName}</span>.
          </SheetDescription>
          <div className="pt-2">
            <button
                ...
                <div className="border rounded-md bg-white dark:bg-slate-950 p-4">
                  <CompetencyRadarChart
                    data={data.quizPerformanceHistory.map((history) => ({
                      subject:
                        history.chapterTitle.length > 15
                          ? history.chapterTitle.slice(0, 15) + '...'
                  ...
                  <div className="flex items-center justify-center gap-x-6 text-xs text-slate-500 mt-2">
                    <div className="flex items-center gap-x-1">
                      <div className="w-3 h-3 bg-indigo-500 opacity-60 rounded-full"></div> Student
                    </div>
                    <div className="flex items-center gap-x-1">
                      <div className="w-3 h-3 bg-slate-300 opacity-60 rounded-full"></div> Class
                      Cohort
                    </div>
                  </div>
                </div>
              ...
              <div className="border rounded-md bg-white dark:bg-slate-950">
                {data.timeline.length === 0 && (
                  <div className="p-4 text-sm text-slate-500">
                    No chapters published in this course.
                  </div>
                )}
                {data.timeline.map((ch, i) => (
                  <div
                    key={ch.id}
                    className={`flex items-start p-4 ${i !== data.timeline.length - 1 ? 'border-b' : ''}`}
                  >
                    <div
                      className={`mt-0.5 mr-4 rounded-full p-1 border ${ch.isCompleted ? 'bg-emerald-100 border-emerald-200 text-emerald-600' : 'bg-slate-100 border-slate-200 text-slate-400'}`}
                    >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <h4
                          className={`text-sm font-medium ${ch.isCompleted ? 'text-slate-900 dark:text-slate-100' : 'text-slate-500'}`}
                        >
                          {ch.title}
                        </h4>
                        {ch.contentType === 'VIDEO_MUX' || ch.contentType === 'VIDEO_YOUTUBE' ? (
                          <Video className="h-3 w-3 text-slate-400" />
                        ) : ch.contentType === 'EVALUATION' ? (
              ...
              {data.quizPerformanceHistory.length === 0 ? (
                ...
                <div className="space-y-3">
                  {data.quizPerformanceHistory.map((log) => (
                    <div
                      key={log.id}
                      className="border rounded-md p-4 bg-white dark:bg-slate-950 flex flex-col space-y-2"
                    >
                      <div className="flex justify-between items-center">
                        <span className="font-semibold text-sm">{log.chapterTitle}</span>
                        <span
                          className={`font-bold text-sm ${log.score >= 70 ? 'text-emerald-600' : 'text-rose-600'}`}
                        >
                          {log.score}%
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-xs text-slate-500">
                        <span>
                          Submitted: {new Date(log.submittedAt).toLocaleDateString()} at{' '}
